...
const AboutArtist = ({ playingSong }: { playingSong: Song | undefined }) => {
  const playingSongArtists = playingSong?.artist?.split(',')
  const artist = artists.find(d => playingSongArtists?.includes(d.name))
